...
int ECDAG::BindXSp(int idx) {
  assert (_ecNodeMap.find(idx) != _ecNodeMap.end()); 
  // 0. create a bind node
  int bindid = _bindId++;
  assert (_ecNodeMap.find(bindid) == _ecNodeMap.end());
  ECNode* bindNode = new ECNode(bindid);
  ...
  vector<int> childids;
  vector<ECNode*> childnodes;
  childnodes = _ecNodeMap[idx]->getChildren();
  ...
  // childids contains the nodeIds, and childnodes contains the child nodes
  // 2. for each node in idxs, figure out corresponding coef
  ECNode* tbnode = _ecNodeMap[idx];
  // add the coef of cur node to the bind node
  for (auto item: tbnode->getCoefmap()) {
    bindNode->addCoefs(bindNode->getNodeId(), item.second);
    bindNode->addCoefs(item.first, item.second);
  }
  // add the ref of cur node to the bind node
  bindNode->setRefNum(idx, tbnode->getRefNumFor(idx));  // it seems no use here
  // clean the childs of cur node and add bind node as child
  tbnode->cleanChilds();
  vector<ECNode*> newChildNodes = {bindNode};
  tbnode->setChilds(newChildNodes);
  tbnode->addCoefs(idx, {1});
  ...
  for (int i=0; i<childnodes.size(); i++) {
    int curcid = childids[i];
    int curref = childnodes[i]->getRefNumFor(curcid);
    childnodes[i]->setRefNum(curcid, curref);
  }
  return bindid;
}
...
void ECDAG::BindY(int pidx, int cidx) {
  ...
  if (consNode->getHasConstraint()) {
    toaddNode->setConstraint(true, consNode->getConsId());
  }
  else{
...
unordered_map<int, AGCommand*> ECDAG::parseForOEC(unordered_map<int, unsigned int> cid2ip,
  ...
  // we can merge the load & cache commands.
  if (w > 1) {
    unordered_map<int, vector<AGCommand *>> sid22merge;
    for (auto item: agCmds) {
      if (item.second->getType() != 2) continue;
      int cid = item.first;
      int sid = cid/w;
      AGCommand *cmdp = item.second; 
      if (sid >= n) continue;
      if (sid22merge.find(sid) == sid22merge.end()) {
        sid22merge.insert(make_pair(sid, vector<AGCommand *>{cmdp}));
      }
      else {
        sid22merge[sid].push_back(cmdp);
      }
    }
    for (auto item: sid22merge) {
      int sid = item.first;
      AGCommand *cmd0 = item.second[0];
      unsigned int sendIP = cmd0->getSendIp();
      string stripeName = cmd0->getStripeName();
      string readobjname = cmd0->getReadObjName();

      vector<int> cidlist;
      unordered_map<int, int> ref;
      for (auto cmditem: item.second) {
        for (auto vecitem: cmditem->getReadCidList()) {
          cidlist.push_back(vecitem);
        }
        for (auto refitem: cmditem->getCacheRefs()) {
          ref.insert(refitem);
        }
        delete cmditem;
      }

      AGCommand *newcmd = new AGCommand();
      newcmd->buildType2(2, sendIP, stripeName, w, num, readobjname,
                                      cidlist, ref);
      for (int wi = 0; wi < w; wi++) {
        agCmds.erase(w*sid + wi);
      }
      agCmds.insert(make_pair(w*sid, newcmd));
    }
  }
...
//     unordered_map<int, int> childId2cid;
//     for (auto item: agCmds) {
//       int cid = item.first;
//       int sid = cid/w;
...
//       if (childId2cid.find(childid) == childId2cid.end()) {
//         tomerge.insert(make_pair(cid, childid));
//         childId2cid.insert(make_pair(childid, cid));
//       }
//       else {
//         if (tomerge.find(childId2cid[childid]) != tomerge.end()) {
//           tomerge.erase(childId2cid[childid]);
//         }
//       }
//     }
