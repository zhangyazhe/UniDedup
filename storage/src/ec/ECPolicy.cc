...
//    toret = new WASLRC(_n, _k, _w, _locality, _opt, _param);
  ...
  } else if (_classname == "LRC32202") {
//    toret = new WASLRC(_n, _k, _w, _locality, _opt, _param);
    toret = new LRC32202(_n, _k, _w, _opt, _param);
  } else if (_classname == "CL262059") {
//    toret = new WASLRC(_n, _k, _w, _locality, _opt, _param);
    toret = new CL262059(_n, _k, _w, _opt, _param);
  } else if (_classname == "RSNSYS") {
    toret = new RSNSYS(_n, _k, _w, _opt, _param);
  } else if (_classname == "RSAS") {
    toret = new RSAS(_n, _k, _w, _opt, _param);
  } else if (_classname == "RSASP") {
    toret = new RSASP(_n, _k, _w, _opt, _param);
  } else if (_classname == "RSRPCONV") {
    if (_smlzSch == "single") {
      toret = new RSRPCONV(_n, _k, _w, _opt, _param);
    }
    else {
      if (_createdRPConvClass < RPNUM) {
        toret = new RSPIPE(_n, _k, _w, _opt, _param);
        if (_smlzRPEnhanced) {
          (static_cast<RSPIPE*>(toret))->setCyclic(_smlzRPEnhanced);
        }
      }
      else {
        toret = new RSNCONV(_n, _k, _w, _opt, _param);
        if (_smlzConvEnhanced) {
          (static_cast<RSNCONV*>(toret))->setConvBindY(_nconvBindy++);
        }
        
      }
      _createdRPConvClass = (_createdRPConvClass + 1) % (RPNUM + CONVNUM);
    }
  } else if (_classname == "RSPPCT") {
...
void ECPolicy::setSMLZSch(string str) {
  _smlzSch = str;
}
void ECPolicy::setSMLZConvEnhanced(bool x) {
  _smlzConvEnhanced = x;
}
void ECPolicy::setSMLZRPEnhanced(bool x) {
  _smlzRPEnhanced = x;
}