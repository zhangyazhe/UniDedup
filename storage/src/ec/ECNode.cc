...
vector<unsigned int> ECNode::candidateIps(unordered_map<int, unsigned int> sid2ip,
                                          ...
                                          int k,
                                          int w,
                                          bool locality, vector<int> lostids) {
  vector<unsigned int> toret;
  int sid = _nodeId/w;
  // cout << "[debug] " << "sid is " << sid << " cid is " << _nodeId << endl;
  // 0. current node has constraint
  if (_hasConstraint) {
    assert(cid2ip.find(_consId) != cid2ip.end());
    toret.push_back(cid2ip[_consId]);
  ...
  // 1. current node is preassigned a location
  if (sid2ip.find(sid) != sid2ip.end() /*&& find(lostids.begin(), lostids.end(), sid) == lostids.end()*/) {
    toret.push_back(sid2ip[sid]);
    return toret;
  }
...
int ECNode::getConsId() {
  return _consId;
}

bool ECNode::getHasConstraint() {
