...
LRC32202::LRC32202(int n, int k, int w, int opt, vector<string> param) {
  _n = n;
  _k = k;
  _w = w;
  _opt = opt;
...
ECDAG* LRC32202::Decode(vector<int> from, vector<int> to) {
  ECDAG* ecdag = new ECDAG();
  if (to.size() == 1) {
...
void LRC32202::generate_matrix(int* matrix, int k, int l, int r, int w) {
  int n = k + l + r;
...
void LRC32202::Place(vector<vector<int>>& group) {
    for (int i = 0; i < _n; i++) {
    vector<int> tmpv;
    tmpv.push_back(i);
