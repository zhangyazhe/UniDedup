...
vector<unsigned int> Coordinator::getCandidates(vector<unsigned int> placedIp, vector<int> placedIdx, vector<int> colocWith) {
  ...
  if (_conf->_fixedLoc) {
    int curIdx = colocWith[0];
    string rack = to_string(curIdx);
    unsigned int dstIp = _conf->_rack2Ips[rack][0];
    toret.push_back(dstIp);
    return toret;
  }
...
//  cout << endl;
  ...
  int pktnum = basesizeMB/_conf->_pktSize;
...
void Coordinator::repairReqFromSS(CoorCommand* coorCmd) {
  ...
  if (redundancy == 0) {
    ...
    if (_conf->_repair_scheduling == "delay") {
      return recoveryOfflineDelay(objname);
    }
    else {
...
void Coordinator::recoveryOffline(string lostobj) {
  struct timeval start_algo, end_algo;
  ...
  vector<int> availcidx;
  vector<int> toreccidx;
  vector<pair<int ,int> > fromcidx2load;
  unordered_map<int, unsigned int> availId2Ip;
  ...
  for (int i = 0; i < ecn; i++)
  {
    ...
    {
        if (_stripeStore->findIfInLostMapOrRPProc(stripeobjs[i])) continue;
        if (_conf->_repairboost)
        {
          string objname = stripeobjs[i];
          SSEntry *curssentry = _stripeStore->getEntryFromObj(objname);
          unsigned int loc;
          loc = curssentry->getLocOfObj(objname);
          int loadt = _stripeStore->getRepairLoad(loc);
          fromcidx2load.push_back(make_pair(i, loadt));
          availId2Ip.insert(make_pair(i, loc));
        }
        else
        {
          for (int j = 0; j < ecw; j++)
  ...
  if (_conf->_repairboost) {
    sort(fromcidx2load.begin(), fromcidx2load.end(),
      [](const pair<int, int> &p1, const pair<int, int> &p2){
        return p1.second < p2.second;
      }
    );
    for (int i = 0; i < eck; i++) {
      for (int j = 0; j < ecw; j++) availcidx.push_back(fromcidx2load[i].first*ecw + j);
    }
    for (int i = 0; i < eck; i++) {
      _stripeStore->increaseRepairLoadMap(availId2Ip[fromcidx2load[i].first], 1);
    }
  }
  ...
  int filesizeMB = ssentry->getFilesizeMB();
  // int objsizeMB = filesizeMB / eck;
  int objsizeMB = _conf->_offlineECBase[ecpoolid];
  // int pktnum = objsizeMB * 1048576/_conf->_pktSize;
  int pktnum = objsizeMB /_conf->_pktSize;
  // parity block
  if (lostidx >= eck) {
    // pktnum = filesizeMB * 1048576/_conf->_pktSize;
    pktnum = filesizeMB /_conf->_pktSize;
  }
  // optimize 
  ecdag->optimize2(opt, cid2ip, _conf->_ip2Rack, ecn, eck, ecw, sid2ip, _conf->_agentsIPs, locality);
  ...
  redisContext* distCtx = RedisUtil::createContext(_conf->_coorIp);
  //sort
  vector<pair<int, AGCommand*>> sortvec;
  for (auto item: agCmds) sortvec.push_back(item);
  sort(sortvec.begin(), sortvec.end(), [](const pair<int, AGCommand*> &c1, const pair<int, AGCommand*> &c2){
    return c1.second->getType() < c2.second->getType();
  });
  ...
  cout << "Repair::algorithm running time: " << RedisUtil::duration(start_algo, end_algo) << endl;
  ...
  cout << "Repair::repair transfer time: " << RedisUtil::duration(start_trans, end_trans) << endl;
  cout << "Coordinator::repair for " << lostobj << " finishes" << endl;
...
void Coordinator::recoveryOfflineDelay(string lostobj0) {
  // cout << "[debug] Delay Repair---------------" << endl; 
  // obtain needed information
  struct timeval start_algo, end_algo;
  gettimeofday(&start_algo, NULL);
  SSEntry* ssentry = _stripeStore->getEntryFromObj(lostobj0);
  string ecpoolid = ssentry->getEcidpool();
  OfflineECPool* ecpool = _stripeStore->getECPool(ecpoolid);
  ecpool->lock();
  ECPolicy* ecpolicy = ecpool->getEcpolicy();
  ...
  // 2, get stripeobjs and lostobj to figure out lostidxs
  string stripename = ecpool->getStripeForObj(lostobj0);
  vector<string> stripeobjs = ecpool->getStripeObjList(stripename);
  vector<string> lostobjs = _stripeStore->getLostObjs(stripename);
  ecpool->unlock();
  vector<int> lostidxs;
  vector<int> integrity;
  for (int i=0; i<stripeobjs.size(); i++) {
    if (find(lostobjs.begin(), lostobjs.end(), stripeobjs[i]) != lostobjs.end()) {
      integrity.push_back(0);
  ...
  _lockSelect.lock();
  for (int i=0; i<stripeobjs.size(); i++) {
    if (find(lostidxs.begin(), lostidxs.end(), i) != lostidxs.end()) {
      for (int j = 0; j < ecw; j++) toreccidx.push_back(i*ecw + j);
    }
    else {
      string objname = stripeobjs[i];
  ...
  // debug
  // cout << "[debug: sid2ip] " << endl ;
  // for (auto i:sid2ip){
  //   in_addr addr;
  //   addr.s_addr = i.second;
  //   cout << "(" << i.first << ", " << inet_ntoa(addr) << ")  ";
  // }
  // cout << endl;
  vector<int> toposeq = ecdag->toposort();
  ...
  // prepare cid2ip, for parseForOEC
  unordered_map<int, unsigned int> cid2ip;
  vector<int> skipcidxs;
  for (int i=0; i<toposeq.size(); i++) {
    int cidx = toposeq[i];
    ECNode* node = ecdag->getNode(cidx);
    if (node->getHasConstraint()) {
      if (cid2ip.find(node->getConsId()) == cid2ip.end()) {
        skipcidxs.push_back(cidx);
        continue;
      }
    }
    vector<unsigned int> candidates = node->candidateIps(sid2ip, cid2ip, _conf->_agentsIPs, ecn, eck, ecw, locality, lostidxs);
    // choose from candidates
    unsigned int curip = chooseFromCandidates(candidates, _conf->_repair_policy, "repair");
    cid2ip.insert(make_pair(cidx, curip));
  }
  for (int i = 0; i < skipcidxs.size(); i++) {
    int cidx = skipcidxs[i];
    ECNode* node = ecdag->getNode(cidx);
    vector<unsigned int> candidates = node->candidateIps(sid2ip, cid2ip, _conf->_agentsIPs, ecn, eck, ecw, locality, lostidxs);
    // choose from candidates
    unsigned int curip = chooseFromCandidates(candidates, _conf->_repair_policy, "repair");
  ...
  gettimeofday(&end_trans, NULL);
  cout << "Repair::repair transfer time: " << RedisUtil::duration(start_trans, end_trans) << endl;
  cout << "Coordinator::repair finishes" << endl;
