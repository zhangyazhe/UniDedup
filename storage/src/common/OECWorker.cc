...
void OECWorker::onlineWrite(string filename, string ecid, int filesizeMB) {
  ...
  int totalNumPkt = filesizeMB /_conf->_pktSize;
...
void OECWorker::offlineWrite(string filename, string ecpoolid, int filesizeMB) {
  ...
  for (int i=0; i<objnum; i++) {
    ...
    int pktnum = sizeMB /_conf->_pktSize;
...
void OECWorker::computeWorker(vector<ECTask*> computeTasks,
  ...
  OECDataPacket** curStripe = (OECDataPacket**)calloc(eck, sizeof(OECDataPacket*));
  OECDataPacket** targetStripe = (OECDataPacket**)calloc(ecn, sizeof(OECDataPacket*));
  ...
  for (int stripeid=0; stripeid<stripenum; stripeid++) {
    ...
    for (int i=0; i<ecn; i++) {
      ...
      if (i < eck) {
        ...
        paritypkt = new OECDataPacket(pktsize);
      }
      targetStripe[i] = paritypkt;
    }
    ...
    for (int i=0; i<ecn; i++) {
      ...
      for (int j=0; j<ecw; j++) {
        int ecidx = i*ecw + j;
        char* bufaddr = pktbuf+j*splitsize;
        tarbufMap.insert(make_pair(ecidx, bufaddr));
      }
    }
    ...
    for (int pktidx=0; pktidx<ecn; pktidx++) {
      objstreams[pktidx]->enqueue(targetStripe[pktidx]);
      targetStripe[pktidx] = NULL;
    }
    // clear data in bufMap
    unordered_map<int, char*>::iterator it = srcbufMap.begin();
    while (it != srcbufMap.end()) {
      int sidx = it->first/ecw;
      if (sidx < eck) {
        curStripe[sidx] = NULL;
      }
    ...
    it = tarbufMap.begin();
    while (it != tarbufMap.end()) {
      int sidx = it->first/ecw;
      if (sidx < ecn) {
        targetStripe[sidx] = NULL;
      } else {
        if (it->second) free(it->second);
      }
      it++;
...
void OECWorker::computeWorkerNonSys(vector<ECTask*> computeTasks, 
  ...
  OECDataPacket** curStripe = (OECDataPacket**)calloc(eck, sizeof(OECDataPacket*));
  OECDataPacket** targetStripe = (OECDataPacket**)calloc(eck, sizeof(OECDataPacket*));
  int pktsize = _conf->_pktSize;
  int splitsize = pktsize / ecw;
  cout << ", pktsize: " << pktsize
       << ", splitsize: " << splitsize
  ...
  for (int stripeid=0; stripeid<stripenum; stripeid++) {
    for (int pktidx=0; pktidx < eck; pktidx++) {
      OECDataPacket* curPkt = readStreams[pktidx]->dequeue();
      curStripe[pktidx] = curPkt;
    }
    // now we have k pkt in a stripe, prepare pkt for parity pkt 
    for (int i=0; i<eck; i++) {
      OECDataPacket* targetpkt = new OECDataPacket(pktsize);
      targetStripe[i] = targetpkt;
    }
    ...
    // now computation is finished, we take out pkt from stripe and put into outputstream
    for (int pktidx=0; pktidx<eck; pktidx++) {
      writeQueue->push(targetStripe[pktidx]);
      targetStripe[pktidx] = NULL;
    }
    // clear data in bufMap
    unordered_map<int, char*>::iterator it = srcbufMap.begin();
...
void OECWorker::readDisk(AGCommand* agcmd) {
  ...
  if (w == 1) {
    // cout << "[debug] in OECWorker::readDisk w == 1." << endl;
    // serail read
  ...
  // else if (w == cidlist.size()) {
  //   // cout << "[debug] in OECWorker::readDisk w == cidlist.size()." << endl; 
  //   // serail read
  //   // read data in serial from disk
  //   thread readThread = thread([=]{objstream->readObj(w, slicesize, remainsize);});
  //   BlockingQueue<OECDataPacket*>* readQueue = objstream->getQueue();
  //   // cacheThread
  //   thread cacheThread = thread([=]{selectCacheWorker(readQueue, num, stripename, w, cidlist, refs);});
  ...
  // }
    ...
    // cout << "[debug] in OECWorker::readDisk w != 1 && w != cidlist.size()." << endl; 
    // random read
...
void OECWorker::partialCacheWorker(BlockingQueue<OECDataPacket*>* cacheQueue,
  ...
  gettimeofday(&time2, NULL);
  cout << "OECWorker::partialCacheWorker.duration: " << RedisUtil::duration(time1, time2) << " for " << keybase << endl;
  redisFree(writeCtx);
}
...
void OECWorker::fetchCompute(AGCommand* agcmd) {
  ...
  int slicesize = _conf->_pktSize/w;
  int remainsize = 0;
  bool ifremain = true;
  for (int i = 0; i < nprevs; i++) {
    if (prevcids[i] % w != 0) {
  ...
  if (ifremain) remainsize = _conf->_pktSize%w;
  thread computeThread = thread([=]{computeWorker(fetchQueue, nprevs, num, coefs, computefor, writeQueue, slicesize+remainsize);});
  ...
  free(writeQueue);
  gettimeofday(&time2, NULL);
  cout << "OECWorker::fetchCompute finishes! duration" << RedisUtil::duration(time1, time2) << endl;
...
void OECWorker::fetchWorker(BlockingQueue<OECDataPacket*>* fetchQueue,
  ...
  for (int i=replyid; i<num; i++) {
    ...
    OECDataPacket* pkt = new OECDataPacket(content);
    int curDataLen = pkt->getDatalen();
    // cout << "OECWorker::fetchWorker.fetch key: " << key << " datalen: " << curDataLen << endl;
    fetchQueue->push(pkt);
    freeReplyObject(rReply);
  }
  gettimeofday(&time2, NULL);
...
void OECWorker::computeWorker(BlockingQueue<OECDataPacket*>** fetchQueue,
  ...
  gettimeofday(&time2, NULL);
  // cout << "[debug] computerWorker finishes" << endl;
  cout << "OECWorker::computeWorker.duration: " << RedisUtil::duration(time1, time2) << endl;
...
void OECWorker::persist(AGCommand* agcmd) {
  ...
  cout << "OECWorker::persist.write as " << objname << " with " << num << " pkts, with nprevs " << nprevs << endl;
  // create fetch queue
  BlockingQueue<OECDataPacket*>** fetchQueue = (BlockingQueue<OECDataPacket*>**)calloc(nprevs, sizeof(BlockingQueue<OECDataPacket*>*));
  for (int i=0; i<nprevs; i++) {
    fetchQueue[i] = new BlockingQueue<OECDataPacket*>();
  ...
  for (int i=0; i<nprevs; i++) {
    string keybase = stripename+":"+to_string(prevcids[i]);
    fetchThreads[i] = thread([=]{fetchWorker(fetchQueue[i], keybase, prevlocs[i], num);cout << "[debug] after fetch thread" << endl;});
  }

  // create objstream and writeThread
  FSObjOutputStream* objstream = new FSObjOutputStream(_conf, objname, _underfs, num*nprevs);
  thread writeThread = thread([=]{objstream->writeObj();cout << "[debug] after write thread" << endl;});
...
//    cout << "OECWorker::persist.left = " << total << endl;
  ...
  redisFree(writeCtx);
  gettimeofday(&time2, NULL);
  cout << "OECWorker::persist finishes.duration: " << RedisUtil::duration(time1, time2) << endl;
...
void OECWorker::readOffline(string filename, int filesizeMB, int objnum) {
  ...
  int pktnum = objsizeMB /_conf->_pktSize;
  for (int i=0; i<objnum; i++) {
    string objname = filename+"_oecobj_"+to_string(i);
...
void OECWorker::readOnline(string filename, int filesizeMB, int ecn, int eck, int ecw) {
  ...
  if (_conf->_is_non_sys_codes) {
    ...
    /* ToDo: 
     * 1. send request to coor
     * 2. receive coefs map and decode
    */
    cout << "OECWorker::readOnline. non sys codes read." << endl;
    ...
    // 0.1 loadn
    cout << "OECWorker::readOnline.non sys codes.loadidx: ";
    int loadn;
    memcpy((char*)&loadn, inststr, 4); inststr += 4;
    loadn = ntohl(loadn);
    vector<int> loadidx;
    ...
    cout << "OECWorker::readOnline.non sys codes inst = " << RedisUtil::duration(time1, time2) << endl;
    ...
    int pktnum = filesizeMB /_conf->_pktSize; 
    thread cacheThread = thread([=]{cacheWorker(writeQueue, filename, pktnum, 1);});

    // 2.1 computeThread
    int stripenum = pktnum/eck;
    thread computeThread = thread([=]{computeWorkerNonSys(computeTasks, readStreams, writeQueue, stripenum, ecn, eck, ecw);});
  ...
  else
  {
    if (!needRecovery)
    {
      ...
      thread cacheThread = thread([=]
                                  { cacheWorker(writeQueue, filename, pktnum, 1); });
    ...
    else
    {
      ...
      // 0.2 computen
      int computen;
      memcpy((char *)&computen, inststr, 4);
      inststr += 4;
      computen = ntohl(computen);
      vector<ECTask *> computeTasks;
      freeReplyObject(instreply);
      ...
      thread computeThread = thread([=]
                                    { computeWorker(readStreams, loadidx, writeQueue, computeTasks, stripenum, ecn, eck, ecw); });
