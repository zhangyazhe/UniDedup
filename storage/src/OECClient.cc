...
int main(int argc, char** argv) {
  ...
  if (reqType == "write") {
    ...
    delete cmd;
    delete conf;
  } else if (reqType == "stopEncode") {
    string confpath(config_path);
    Config* conf = new Config(confpath);    
    // send coorCmd to coordinator?
    CoorCommand* cmd = new CoorCommand();
    cmd->buildType7(7, 0, "encode");
    cmd->sendTo(conf->_coorIp);
    ...
    delete cmd;
    delete conf;
  } else if (reqType == "stopRepair") {
    string confpath(config_path);
    Config* conf = new Config(confpath);
    // send coorCmd to coordinator
    CoorCommand* cmd = new CoorCommand();
    cmd->buildType7(7, 0, "repair");
    cmd->sendTo(conf->_coorIp);
    delete cmd;
    delete conf;
