...
void chunkAlg_init() {
	...
	windows_reset();
	_num_chunks = 0;
	rabin_mask = chunkMetaData.chunk_avg_size - 1;
}
...
int rabin_chunk_data(unsigned char *p, int n) {
	...
	if (n <= chunkMetaData.chunk_min_size)
		return n;
	else
		i = chunkMetaData.chunk_min_size;

	int end = n > chunkMetaData.chunk_max_size ? chunkMetaData.chunk_max_size : n;
	while (i < end) {
...
int normalized_rabin_chunk_data(unsigned char *p, int n) {
	...
	int small_mask = chunkMetaData.chunk_avg_size*2 - 1;
	int large_mask = chunkMetaData.chunk_avg_size/2 - 1;
	int end = n > chunkMetaData.chunk_max_size ? chunkMetaData.chunk_max_size : n;
	while (i < end) {

		SLIDE(p[i - 1], fp, bufPos, buf);

		if (i < chunkMetaData.chunk_avg_size) {
...
int tttd_chunk_data(unsigned char *p, int n) {
	...
	int back_mask = chunkMetaData.chunk_avg_size/2 - 1;
	int end = n > chunkMetaData.chunk_max_size ? chunkMetaData.chunk_max_size : n;
	while (i < end) {
